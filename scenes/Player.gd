...
func _integrate_forces(state):
	...
	state.linear_velocity.x = lerp (state.linear_velocity.x, dir.x * 1000 * state.step, 0.01 )
	state.linear_velocity.z = lerp (state.linear_velocity.z, dir.z * 1000 * state.step, 0.01 )
